...
describe("PDFImage", function () {
  ...
  before(function (done) {
    ...
    if (fs.existsSync(pdfPath)) {
      ...
      throw new Error({
        message:
          "File missing at: " + pdfPath + ". Copy task was not a success",
      });
    }
  });
  ...
  it("should return correct page path", function () {
    expect(pdfImage.getOutputImagePathForPage(1)).equal("/tmp/test-1.png");
    expect(pdfImage.getOutputImagePathForPage(2)).equal("/tmp/test-2.png");
    expect(pdfImage.getOutputImagePathForPage(1000)).equal(
      "/tmp/test-1000.png"
    );
    expect(pdfImage.getOutputImagePathForFile()).equal("/tmp/test.png");
  });

  it("should return correct convert command", function () {
    expect(pdfImage.constructConvertCommandForPage(1)).equal(
      'convert "/tmp/test.pdf[1]" "/tmp/test-1.png"'
    );
  });

  it("should return correct convert command to combine images", function () {
    expect(
      pdfImage.constructCombineCommandForFile([
        "/tmp/test-0.png",
        "/tmp/test-1.png",
      ])
  ...
  it("should use gm when you ask it to", function () {
    ...
    expect(pdfImage.constructConvertCommandForPage(1)).equal(
      'gm convert "/tmp/test.pdf[1]" "/tmp/test-1.png"'
    );
  });
